...
import emma.galzio.fido2server.common.AuthenticatorTransport;
import emma.galzio.fido2server.common.TokenBinding;
import emma.galzio.fido2server.common.UserVerificationRequirement;
import emma.galzio.fido2server.common.crypto.Digests;
...
import emma.galzio.fido2server.helper.CredentialPublicKeyHelper;
import emma.galzio.fido2server.helper.ExtensionHelper;
import emma.galzio.fido2server.helper.SignatureHelper;
import emma.galzio.fido2server.model.*;
import emma.galzio.fido2server.repository.IUserRepository;
import emma.galzio.fido2server.util.AaguidUtil;
...
public class ResponseServiceImpl extends ResponseCommonService implements ResponseService {
    ...
    public RegisterCredentialResult handleAttestation(ServerRegPublicKeyCredential serverPublicKeyCredential, String sessionId,
        ...
        return getRegisterCredentialResult(session.getRegOptionResponse(), attestationResponse.getTransports(), attestationObject.getAuthData(),
                attestationVerificationResult, serverPublicKeyCredential.getExtensions(), rpId, session.getUser());
    }

    protected RegisterCredentialResult getRegisterCredentialResult(RegOptionResponse regOptionResponse, List<AuthenticatorTransport> transports,
